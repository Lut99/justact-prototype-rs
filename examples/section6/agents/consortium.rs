...
impl Synchronizer<(String, u32), (String, char), Program, u64> for Consortium {
    type Error = Error;
        ...
        SA: ConstructableAction<Id = (String, char), ActorId = Self::Id, Message = SM, Timestamp = u64>,
    {
        let mut handler = self.handler.handle(view)
            ...
            .on_start(|view| -> Result<(), Error> {
                let message: SM = create_message(1, ID, slick::parse::program(include_str!("../slick/consortium_1.slick")).unwrap().1);
        ...
        if self.script == Script::Section6_3_5 {
            handler = handler
                ...
                .on_enacted((super::st_antonius::ID, 'a'), |view, _| -> Result<(), Error> {
                    // Push the amendment
                    let message: SM = create_message(2, ID, slick::parse::program(include_str!("../slick/consortium_2.slick")).unwrap().1);
                    view.stated.add(Recipient::All, message.clone()).cast()?;
                ...
                .on_tick_to(3, |view| -> Result<(), Error> {
                    view.agreed.add(Agreement { message: create_message(2, ID, slick::parse::program(include_str!("../slick/consortium_2.slick")).unwrap().1), at: 3 }).cast()?;
                    Ok(())
                })?;
        }
